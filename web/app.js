...
const API_BASE = (location.search.match(/api=([^&]+)/) || [])[1] || 'http://127.0.0.1:8000';
...
async function analyzeSingle() {
  ...
  const res = await fetch(`${API_BASE}/analyze-single`, { method:'POST', body: fd });
  if (!res.ok) { setStatus('Error analyzing file'); return; }
...
function renderSingleResult(file, data) {
  ...
  renderEQ(qs('#eq-chart'), qs('#eq-text'), data.eq_data, r.primary_emotion);
...
async function analyzeBatch() {
  ...
  if (!res.ok) { setStatus('Batch analysis failed'); return; }
  const data = await res.json();
...
function renderBatchResult(files, data) {
  ...
  qs('#songs').textContent = r.total_songs;
  qs('#dom-emotion').textContent = (r.dominant_emotion||'').toUpperCase();
  qs('#avg-valence').textContent = (r.average_valence||0).toFixed(2);
  qs('#avg-arousal').textContent = (r.average_arousal||0).toFixed(2);
  ...
  renderEQ(qs('#eq-chart'), qs('#eq-text'), data.eq_data, r.dominant_emotion);
